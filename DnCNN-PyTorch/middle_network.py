...
class BRDNet(nn.Module):
    def __init__(self, channels):
        ...
        # 第18层
        # self.conv1 = nn.Sequential(nn.Conv2d(in_channels=channels*3, out_channels=channels, kernel_size=kernel_size, padding=padding,
        #               groups=groups, bias=False))
    def forward(self, x):
        # out1 = self.BRDNet_first(x)
        out = self.BRDNet_second(x)
        # out3 = self.BRDNet_third(x)
        # out1 = x - out1
        # out2 = x - out2
        # out3 = x - out3
        # out = torch.cat([out1, out2,out3], 1)
        # out = self.conv1(out2)    ## out 为 残差图像，近似为 噪声
